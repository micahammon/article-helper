...
class LearnerFacingLanguageTests(unittest.TestCase):
    ...
    def test_each_page_carries_the_same_rules_as_its_data_file(self):
        """Both pages hold a copy so they work from a file:// document.

        The JSON files are where the rules are written; tools/inline_rules.py
        ...
        for data_name, page_name in (("rules_data.json", "index.html"),
                                     ("rules_data.classic.json", "classic.html")):
            page = (root / page_name).read_text(encoding="utf-8")
            start = page.find(opener)
            self.assertNotEqual(start, -1, f"{page_name} has no rules-data block")
            start += len(opener)
            inlined = json.loads(page[start:page.find("</script>", start)])
            source = json.loads((root / data_name).read_text(encoding="utf-8"))
            self.assertEqual(inlined, source,
                             f"{page_name} is out of date: run tools/inline_rules.py")
